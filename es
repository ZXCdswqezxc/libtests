...
local esp = {
    ...
    fixed_box_width = 4,       -- ширина фиксированного бокса
    fixed_box_height = 6,      -- высота фиксированного бокса
    
    -- настройка толщины полосок здоровья
    healthbar_thickness = 1,   -- толщина полоски здоровья
...
-- Добавляем функцию для создания фиксированных offsets
function esp:returnFixedOffsets(character)
    local centerMass = character.HumanoidRootPart.CFrame
    local width = self.fixed_box_width / 2
    local height = self.fixed_box_height / 2
    local depth = width -- используем ширину для глубины тоже
    
    return {
        NEWCF(width, height, depth),    -- верхний правый передний
        NEWCF(-width, height, depth),   -- верхний левый передний
        NEWCF(width, height, -depth),   -- верхний правый задний
        NEWCF(-width, height, -depth),  -- верхний левый задний
        NEWCF(width, -height, depth),   -- нижний правый передний
        NEWCF(-width, -height, depth),  -- нижний левый передний
        NEWCF(width, -height, -depth),  -- нижний правый задний
        NEWCF(-width, -height, -depth)  -- нижний левый задний
    }
end
...
function esp:update()
    for plr, drawing in next, esp.players do
        ...
        if esp.enabled and esp.checkalive(player) then
            ...
            if drawing.arrow.Visible then
                ...
                if esp.arrowinfo then
                    ...
                    if drawing.arrow_bar.Visible then
                        ...
                        drawing.arrow_bar.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, ( - health / 100 * ( biggestY - smallestY + 2)) + 3))
                        drawing.arrow_bar.Position = esp:floorvector(NEWVEC2(smallestX - esp.healthbar_thickness - 2, smallestY + drawing.arrow_bar_outline.Size.Y))
                        drawing.arrow_bar.Transparency = transparency
                        drawing.arrow_bar_inline.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, ( - 1 * ( biggestY - smallestY + 2)) + 3))
                        drawing.arrow_bar_inline.Position = drawing.arrow_bar.Position
                        drawing.arrow_bar_inline.Transparency = transparency
                        drawing.arrow_bar_outline.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, biggestY - smallestY))
                        drawing.arrow_bar_outline.Position = esp:floorvector(NEWVEC2(smallestX - esp.healthbar_thickness - 1, smallestY + 1))
            ...
            -- Проверяем, используем ли фиксированные размеры или оригинальные расчеты
            local offsets
            
            if esp.fixed_size_enabled then
                -- Используем фиксированные размеры на основе HumanoidRootPart
                offsets = esp:returnFixedOffsets(character)
            else
                -- Оригинальный расчет размеров на основе конечностей персонажа
                ...
                offsets = esp:returnoffsets(minX, y, minY, character['HumanoidRootPart'].Size.Z / 2)
            end
            ...
            if drawing.bar.Visible then
                ...
                drawing.bar.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, ( - health / 100 * ( biggestY - smallestY + 2)) + 3))
                drawing.bar.Position = esp:floorvector(NEWVEC2(smallestX - esp.healthbar_thickness - 2, smallestY + drawing.bar_outline.Size.Y))
                drawing.bar.Transparency = transparency
                drawing.bar_inline.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, ( - 1 * ( biggestY - smallestY + 2)) + 3))
                drawing.bar_inline.Position = drawing.bar.Position
                drawing.bar_inline.Transparency = transparency
                drawing.bar_outline.Size = esp:floorvector(NEWVEC2(esp.healthbar_thickness, biggestY - smallestY))
                drawing.bar_outline.Position = esp:floorvector(NEWVEC2(smallestX - esp.healthbar_thickness - 1, smallestY + 1))
